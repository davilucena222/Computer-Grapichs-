...
material.fragmentShader = `
    ...
    //variáveis para realizar o cálculo de cada termo
    ...
    void main() {
        ...
        //Cálculos para realizar a coloração de acordo com o modelo de iluminação de Phong
        
        //calculando o termo ambiente
        vec3 termo_ambiente = Ip_ambient_color * k_a;       
        
        //calculando o termo difuso
        vec3 termo_difuso = Ip_diffuse_color * k_d * max(vec3(0.0, 0.0, 0.0), dot(N_cam_spc, L_cam_spc)); //calculando o termo difuso
        
        //calculando o termo especular
        vec3 termo_especular = Ip_diffuse_color * k_s * pow(max(vec3(0.0, 0.0, 0.0), dot(R_cam_spc, normalize(-L_cam_spc))), vec3(3.0, 3.0, 3.0));
        
        //calculando a soma total das três variáveis de cada termo
        vec3 somatorio_total = termo_ambiente + termo_difuso + termo_especular; 
        
        // 'gl_FragColor' : variável de sistema que conterá a cor final do fragmento calculada pelo Fragment Shader.
        
        //Passando a variável como parâmetro para realizar o cálculo da cor final dos fragmentos
        gl_FragColor = vec4(somatorio_total, 1);
    }
    `;
